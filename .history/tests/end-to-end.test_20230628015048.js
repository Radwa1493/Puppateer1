...
describe('Registration Test', () => {
  ...
  it('Open Profile Page', async () => {
    ...
    await profilePage.isLoginPageDisplayed(); //wait for logedin element
  });
  ...
  it('Open Registration Page', async () => {
    ...
    await registionPage.isRegisterPageDisplayed(); //wait Registration page opened
  });
...
describe('Login Test', () => {
  ...
  // Test case enter a valid email and valid password
  it('Enter valid credentials', async function () {
    console.log('email :' + registionPage.getmail()); // display a valid email
...
describe('Shopping  Test', () => {
  ...
  */
  it('Verify Basket Data', async function () {
    await header.GotoCart(); // go to cart page
    await cartPage.GetCartPrice(); 

    //Assert on actual and expected price
    expect(cartPage.GetAcualtotalprice()).toEqual(catPage.GetExpectedPrice());
    //Assert on actual and expected items
    for (let c = 0; c < catPage.Getexpecteditems().length; c++) {
