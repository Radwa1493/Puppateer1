...
const elements = {
 ...
 totalPrice:'div.cartOverview__summaryContainer:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(6) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1)';
...
export default class CartPage extends Base {
  ...
  async GetCartPrice() {
    ...
    const Exprice1 = await this.getText(page, elements.totalPrice); // get  price in text
    let Exprice2 ="";

    if (await this.isElementDisplayed(elements.fraction)) {
       Exprice2 = await this.getText(page, elements.fraction).replace("-", ""); // get .00 price in text
    }

    const Exprice3 =Exprice1.replace(".", "")+"."+ Exprice2; // get total price in text
    const Exprice4 =  parseFloat(Exprice3);  
