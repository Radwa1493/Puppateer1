...
export default class CatPage extends Base {
  ...
  async selectRandamItem() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const price = GetText(priceElement); // get price in text
      const itemName1Text = GetText(name1Element); // get item name1 in text
      //    const itemName2Text =GetText(name1Element); name2Element);// get item name1 in text
