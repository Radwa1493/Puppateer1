...
export default class CatPage extends Base {
  ...
  async selectRandamItem() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const priceFactor = await this.getText(page, elements.pricefactor); // get item pricefactor in text
      ...
      if (priceFactor !== "-") { priceFactor = Number(priceFactor)}

      totalprice = totalprice +priceFactor+ getNumberAfterComma(price);
