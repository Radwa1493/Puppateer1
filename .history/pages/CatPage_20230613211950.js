...
export default class CatPage extends Base {
  ...
  async selectRandamItem() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const price = this.GetText(priceElement); // get price in text
      const itemName1Text = this.GetText(name1Element); // get item name1 in text
      //    const itemName2Text =this.GetText(name2Element);// get item name1 in text
